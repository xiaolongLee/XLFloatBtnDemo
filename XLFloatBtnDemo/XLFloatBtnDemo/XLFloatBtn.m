...
@implementation XLFloatBtn{
    
    XLAssistiveTouchType  _type;
    //拖动按钮的起始坐标点
    CGPoint _touchPoint;
...
+ (instancetype)getFloatBtn{
    if (!_floatBtn) {
        _floatBtn = [[self alloc]initWithType:XLAssistiveTypeNearRight frame:CGRectZero];
    }
    return _floatBtn;
}
...
+ (void)show{
    ...
    [self showWithType:XLAssistiveTypeNearRight];
}
...
+ (void)showWithType:(XLAssistiveTouchType)type{
    ...
    dispatch_once(&onceToken, ^{
        ...
        _floatBtn = [[XLFloatBtn alloc] initWithType:XLAssistiveTypeNearRight frame:CGRectZero];
    });
...
- (instancetype)initWithType:(XLAssistiveTouchType)type
                       frame:(CGRect)frame{
...
//    UIImage *image = [UIImage imageWithContentsOfFile:path];
    ...
    UIImage *image =  [UIImage imageNamed:@"mn_placeholder.png"];
